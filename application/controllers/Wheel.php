...
class Wheel extends CI_Controller {
        ...
        if (($registrations == $numberOfLeads || $registrations > $numberOfLeads) && $isUnlimitedLeads != 1) {
            ...
            /**
            	...
            	- because in scratch, email id is unique for user and 
            		in wheel mobile number is unique so I did not make a separate field for each.
    ...
    function loadGameInstant(){
        ...
        $campaignId = decrypt($this->input->post('campaignId'));
        $numberOfTimeSpin = $this->input->post('numberOfTimeSpin');
        $isWinningForm = 0;
        ...
        $secondChancePercentage = $getCampaingDetail['wheelMultiImagePercentage8'];
        ...
        if ($secondChancePercentage > 0) {                    
            ...
            //now check if random number is lesser than secondChance percentage

            if ($secondChanceRandom < $secondChancePercentage) {
                
                //make array of second chance
                $returnData = array('isSecondChance' => 1,'isConfettiOn' =>$getCampaingDetail['isConfettiOnSecondChance'],'serverDegree' => 90);
        ...
        //get winning form data
        if ($isWinningForm == 1) {
            $wheelWinningForm = $this->getWheelWinningStuff($campaignId);
            $returnData = array('isSecondChance' => 0,'wheelWinningForm' => $wheelWinningForm);
            echo json_encode($returnData);
        }
           
    }

    function getWheelStuff(){
        ...
        if($getCampaingDetail['appNameIndex'] != 3){        
                ...
                if (count($getUserDetail) > 0) {   
                    ...
                    if ($secondChancePercentage > 0) {
                        ...
                        /*
                            ...
                            2. check if user already its second chance or not
            ...
            $wheelWinningForm = $this->getWheelWinningStuff($campaignId,$numberOfTimeSpin);
            $returnData = array('isSecondChance' => 0,'wheelWinningForm' => $wheelWinningForm);
            echo json_encode($returnData);
        }    
    }

    function getWheelWinningStuff($campaignId = 0,$numberOfTimeSpin = 0){
    	...
    	*/
    	for ($i = 0; $i < 7; $i++) { 
    		$percentage[$i] = $getCampaingDetail['wheelMultiImagePercentage'.($i + 1)];
    	}
    	...
    	*/
    	...
    	/**
    		Now, make one random number and check it lies in which range 
    	*/
    	...
    	/**
    		For easy to understand to other developer I'm taking below array
    		There is also other way to do below thing such as just add +1 in $rangeIndex
    	*/

        $winningFormIndexArr = [1,2,3,4,5,6,7];
        if($getCampaingDetail['appNameIndex'] == 1){
            $degreeArr = [45,360,315,270,225,180,135]; 
        }else if($getCampaingDetail['appNameIndex'] == 3){
            $degreeArr = [315,270,225,180,135,90,45]; 
        }      
    	...
    	/**
    		Feed array
    	*/
    	$winningFormField = array('wheelWinningImageForm','isConfettiOnForm','isBgTransparent','isHideScarcityBar','isHideCountDown','wheelWinningHeader','wheelWinningHeaderBGColor','wheelWinningHeaderStyle','wheelWinningHeaderFontColor','wheelWinningDescriptionBeforeImage','wheelWinningDescriptionBeforeImageStyle','wheelWinningDescriptionBeforeImageFontColor','wheelWinningDescription','wheelWinningDescriptionStyle','wheelWinningDescriptionFontColor','wheelWinningValueDescription','wheelWinningValueDescriptionBGColor','wheelWinningValueDescriptionStyle','wheelWinningValueDescriptionFontColor','wheelScarcityBarText','wheelScarcityCountDown','wheelScarcityBarBGColor','wheelWinningScarcityBarFontStyle','wheelWinningScarcityBarFontColor','wheelWinningButtonText','wheelWinningButtonBGColor','wheelWinningButtonStyle','wheelWinningButtonFontColor','wheelWinningButtonLink','wheelWinningAgainButtonText','wheelWinningAgainButtonBGColor','wheelWinningAgainButtonStyle','wheelWinningAgainButtonFontColor','wheelWinningTermsText','wheelWinningTermsLink','wheelWinningTermsBGColor','wheelWinningTermsStyle','wheelWinningTermsFontColor');

        $winningFormValue = array();
        // change index if wheel type is number order instead of percentage
        if($getCampaingDetail['appNameIndex'] == 1){  
            
            $winningFormValue['formIndex'] = $winningFormIndexArr[$rangeIndex];
            $winningFormValue['serverDegree'] = $degreeArr[$rangeIndex];
            $winningFormValue['action'] = "percentage";
            $winningIndex = $rangeIndex;

        }else if($getCampaingDetail['appNameIndex'] == 3){  

            if($numberOfTimeSpin == 1){
                $winningFormValue['formIndex'] = $winningFormIndexArr[$rangeIndex];
                $winningFormValue['serverDegree'] = $degreeArr[$rangeIndex];
                $winningFormValue['action'] = "percentage";
                $winningIndex = $rangeIndex;
                ...
                $wheelOrder = array(
                    '0' => $getCampaingDetail['wheelMultiImageOrder1'],
                    '1' => $getCampaingDetail['wheelMultiImageOrder2'],
                    '2' => $getCampaingDetail['wheelMultiImageOrder3'],
                    '3' => $getCampaingDetail['wheelMultiImageOrder4'],
                    '4' => $getCampaingDetail['wheelMultiImageOrder5'],
                    '5' => $getCampaingDetail['wheelMultiImageOrder6'],
                    '6' => $getCampaingDetail['wheelMultiImageOrder7']
                );
                asort($wheelOrder);

                $numberIndex = array_search($numberOfTimeSpin-1,$wheelOrder);            
                $winningFormValue['formIndex'] = $winningFormIndexArr[$numberIndex];
                $winningFormValue['serverDegree'] = $degreeArr[$numberIndex];
                $winningFormValue['action'] = "number";
                $winningIndex = $numberIndex;
            }                      
    	...
    	for ($i=0; $i < count($winningFormField); $i++) { 
    		...
    		$winningFormValue[$winningFormField[$i]] = $getCampaingDetail[$winningFormField[$i].$winningFormIndexArr[$winningIndex]];
    	}
        ...
        // add congrats config details details 
        $winningFormValue['congratsText'] = $getCampaingDetail['congratsText'];
        $winningFormValue['congratsColor'] = $getCampaingDetail['congratsColor'];
        $winningFormValue['congratsStyle'] = $getCampaingDetail['congratsStyle'];
    ...
    function declareUserWin(){
        ...
        /**
            ...
            In wheel, mobile number is compulsory
